    ...
    def __init__(self, server_address, RequestHandlerClass, bind_and_activate=True):
        ...
        try:
            ...
            if self.address_family == socket.AF_INET6:
                ...
                print(f"[警告] 绑定 Web 管理后台 IPv6 {host}:{port} 失败 ({e})，正在尝试回退�?IPv4 {fallback_host} ...", flush=True)
...
def env_int(name: str, default: int, min_value: int | None = None, max_value: int | None = None) -> int:
    ...
    try:
        ...
        print(f"[配置警告] 环境变量 {name}={raw!r} 不是有效整数，使用默认�?{default}", flush=True)
        value = default
    if min_value is not None and value < min_value:
        print(f"[配置警告] 环境变量 {name}={value} 小于允许�?{min_value}，使用默认�?{default}", flush=True)
        return default
    if max_value is not None and value > max_value:
        print(f"[配置警告] 环境变量 {name}={value} 大于允许�?{max_value}，使用默认�?{default}", flush=True)
        return default
    return value
...
# 初始化时优先�?ui_auth.json 加载保存的代理出站端口和网页端口配置以覆盖环境变�?try:
            ...
            if match:
                ...
                try:
                    ...
                    if today_time - file_time >= three_days_sec:
                        with lock:
                            path.unlink()
                        print(f"[清理] 已删�?天前的旧日志文件: {path.name}", flush=True)
                except Exception:
                    if now - path.stat().st_mtime > three_days_sec:
                        with lock:
                            path.unlink()
    except Exception as e:
        print(f"[清理错误] 清理旧日志失�? {e}", flush=True)
...
def fetch_api_text(url: str | None = None, use_ssl_verify: bool = True) -> str:
    ...
    if ptype and phost and pport:
        try:
            print(f"[fetch_api_text] 监测到上游代�?({ptype}://{phost}:{pport})，尝试通过代理获取 API...", flush=True)
            return fetch_api_text_via_proxy(url, ptype, phost, pport, use_ssl_verify)
        except Exception as e:
            print(f"[fetch_api_text] 通过代理获取 API 失败: {e}，尝试使用直�?默认系统代理...", flush=True)
...
def fetch_candidates() -> list[dict[str, Any]]:
    ...
    # 检查本地是否有节点缓存，以确定最大重试尝试次�?    has_cache = len(cached_nodes()) > 0
    max_attempts = 1 if has_cache else 2
    
    # 尝试 URLs 队列: 1. HTTPS(验证证书) 2. HTTPS(不验证证�? 3. HTTP
    attempts_targets = [
        (API_URL, True),
        (API_URL, False)
    ]
    ...
    log_to_json("INFO", "Main", "开始拉取官�?API 节点列表...")
    ...
    for url, verify_ssl in attempts_targets:
        for i in range(max_attempts):
            if i > 0:
                time.sleep(1.5)
            try:
                msg = f"尝试拉取 {url} (SSL验证: {verify_ssl}, �?{i+1} 次尝�?..."
                ...
                for row in rows[:MAX_SCAN_ROWS]:
                    ...
                    try:
                        ...
                        print(f"[fetch_candidates] 跳过损坏的节点配置记�? {row_exc}", flush=True)
                        log_to_json("WARNING", "Main", f"跳过损坏的节点配置记�? {row_exc}")
                        continue
    ...
    if not candidates:
        ...
        full_err_msg = f"获取官方 API 节点最终失�? {last_err} | 诊断结果: {diag_msg}"
    ...
    log_to_json("INFO", "Main", f"成功获取官方 API 节点，共 {len(candidates)} 个候选节�?)
    return candidates
...
def update_handshake_status(line_lower: str) -> None:
    status_map = {
        ...
        "peer connection initiated": ("协商加密", "控制通道已建立，已初始化与服务器的加密对等连�?.."),
        "push_request": ("请求配置", "正在向服务器发�?PUSH_REQUEST 请求配置参数�?IP 分配..."),
        "push_reply": ("应用配置", "已接收服务器 PUSH_REPLY，获取到 IP 分配，正在准备配置网�?.."),
        "tun/tap device": ("创建网卡", "正在创建虚拟通道并打开 TUN 虚拟网卡设备..."),
        "do_ifconfig": ("网卡配置", "正在为虚拟网卡配�?IP 地址及相关网络属�?.."),
    }
...
def run_openvpn_until_ready(config_file: str, keep_alive: bool, route_nopull: bool, timeout: int | None = None, dev: str = "tun0") -> tuple[bool, str, subprocess.Popen[str] | None]:
    ...
    try:
        ...
        return False, "[错误代码 2001] [ERR_OVPN_CMD_NOT_FOUND] 未找�?openvpn 命令。原�? 系统未安�?openvpn，或 PATH 环境变量不正确�?, None
    except OSError as exc:
        return False, f"[错误代码 2002] [ERR_OVPN_START_FAILED] openvpn 启动失败: {exc}。原�? 系统权限不足或配置冲突�?, None
    ...
    if not ok:
        ...
        message = f"[错误代码 {err_code}] {diag_msg} (原始日志尾部: {tail[-1][-100:] if tail else '�?})"
    startup_done[0] = True
...
def setup_policy_routing(interface: str = "tun0", table: int = 100, channel_idx: int = 0) -> None:
    ...
    for attempt in range(1, 4):
        try:
            ...
            proxy_port = CHANNEL_BASE_PORT + channel_idx
            try:
    ...
    if not success:
        print(f"[路由配置失败] 无法向路由表 {table} 添加默认路由 (接口 {interface})。请检�?root 权限�?, flush=True)
    ...
    with test_indexes_lock:
        for idx in range(MAX_CHANNELS + 2, 100):
            if idx not in active_test_indexes:
                ...
                return idx
        raise RuntimeError("没有可用�?OpenVPN 测试网卡编号，请稍后重试")
...
def test_multiple_nodes(node_ids: list[str]) -> list[dict[str, Any]]:
    ...
    # 批量查询并丰富可用节点的地理�?ISP 信息，防止并发时被定�?API 接口限流
...
def auto_switch_node(attempt: int = 0) -> None:
    if attempt >= 3:
        print("[自动切换] 连续切换失败已达 3 次，停止切换以防止主线程死锁，将在后台重新加载节�?..", flush=True)
        return
    ...
    if candidates:
        ...
        msg = f"当前连接已失效或代理连通性检测失败，正在自动切换至最佳备用节�? {next_node['id']}"
        print(f"[自动切换] {msg}", flush=True)
        log_to_json("INFO", "VPN", msg)
        try:
            connect_node(next_node["id"])
        except Exception as e:
            err_msg = f"切换到备用节�?{next_node['id']} 失败: {e}，将尝试下一�?.."
        ...
        def bg_fetch_and_switch():
            try:
                ...
                print(f"[自动切换后台补齐] 获取并测试节点失�? {e}", flush=True)
...
def connect_node(node_id: str) -> str:
    ...
    with lock:
        if is_connecting:
            print("[连接] 正在建立其他连接中，跳过此请�?, flush=True)
            raise RuntimeError("当前已有连接或节点检测任务正在运行，请稍后再�?)
        is_connecting = True
        set_state(is_connecting=True, active_node_latency="正在连接", last_check_message=f"正在初始化连接配�? {node_id}")
        
    try:
        log_to_json("INFO", "VPN", f"开始连接节�? {node_id}")
        ...
        set_state(active_node_latency="清理连接", last_check_message="正在关闭与清理旧�?VPN 连接及网�?..")
        stop_active_openvpn()
        ...
        set_state(active_node_latency="启动核心", last_check_message="正在启动 OpenVPN Core 核心服务并建立连�?..")
        ...
        if not ok or process is None:
            ...
            print(f"[连接核心失败] 无法�?VPN 节点 {node_id} 建立隧道连接！详�? {message}", flush=True)
            set_state(active_openvpn_node_id="", is_connecting=False, active_node_latency="无活动连�?, last_check_message=f"连接失败: {message}")
            with lock:
                active_openvpn_node_id = ""
        ...
        set_state(active_node_latency="配置路由", last_check_message="正在配置策略路由规则与流量转�?..")
        ...
        set_state(active_node_latency="测试延迟", last_check_message="正在直连测试代理出口延迟与可用�?..")
        try:
        ...
        latency_str = f"{last_active_latency} ms" if last_active_latency > 0 else "检测超�?
        ...
        log_to_json("INFO", "VPN", f"节点 {node_id} 连接成功，出口网�?tun0 已启�?)
...
def maintain_valid_nodes(force: bool = False) -> str:
    ...
    try:
                ...
                if routing_mode == "fixed_ip":
                    ...
                    if target_id:
                        ...
                        if any(n.get("id") == target_id for n in nodes):
                            print(f"[维护线程] 检测到固定 IP 模式�?OpenVPN 未运行，正在重新拉起同一节点: {target_id}", flush=True)
                            is_connecting = False
                            try:
        ...
        msg = f"开始对列表中所有候选节点进行周期连通性与延迟测试，待检测节点共 {len(to_test_ids)} �?
        print(f"[周期检测] {msg}", flush=True)
        log_to_json("INFO", "Main", msg)
        
        set_state(is_connecting=True, last_check_message="正在并发检测所有节点可用�?..")
        ...
        with lock:
            ...
            active_node = next((n["id"] for n in merged if n.get("active")), "�?)
            
            status_report = (
                f"周期节点检测完成。实时同步状�? 获取到候选节点共 {len(merged)} 个�?"
                f"其中【可用节点】{len(available_nodes)} �? {available_nodes[:15]}...; "
                f"【不可用节点】{len(unavailable_nodes)} �? "
                f"当前【正在正常运行的活动连接节点】为: {active_node}�?
            )
            print(f"[周期检测] {status_report}", flush=True)
            log_to_json("INFO", "Main", status_report)
            
            if active_node != "�? and not active_openvpn_running():
                warn_msg = f"[诊断警告] 活动节点 {active_node} 被标记为活动状态，�?OpenVPN 进程实际并未正常运行�?
...
def collector_loop() -> None:
    ...
    while True:
        ...
        success = False
        try:
            print("[守护线程] 开始执行节点拉取与可用性检测周期任�?..", flush=True)
            log_to_json("INFO", "Main", "开始执行节点拉取与可用性检测周期任�?..")
...
<body>
  <div class="login-container">
    <div class="login-card">
      ...
      <p class="login-subtitle">请输入您的管理账号和安全密码以继�?/p>
      
      <form id="login_form" onsubmit="handleLogin(event)">
        <div class="form-group">
          <label class="form-label" for="username">管理账号</label>
          <div class="input-wrapper">
            <input type="text" id="username" name="username" class="input-field" placeholder="请输入管理账�? required autocomplete="username">
          </div>
        </div>
        <div class="form-group" style="margin-top: 16px;">
          <label class="form-label" for="password">安全密码</label>
          <div class="input-wrapper">
            <input type="password" id="password" name="password" class="input-field" placeholder="请输入安全密�? required autocomplete="current-password">
          </div>
  ...
  <script>
    async function handleLogin(e) {
      ...
      try {
        ...
        errorText.textContent = "连接服务器失败，请稍后重�?;
        errorText.style.display = "block";
        submitBtn.disabled = false;
        submitBtn.querySelector("span").textContent = "登录";
      }
...
<span class="status-badge" id="systemStatus"><span class="dot"></span>系统运行�?/span>
</div>
...
<a onclick="openGatewayModal()">服务状�?/a>
<a onclick="openLogsModal()">运行日志</a>
<a onclick="openVpsModal()">推荐 VPS</a>
<a onclick="logoutAdmin()" style="color:#fb7185">退出登�?/a>
</div>
</div>
</div>
</header>
...
<main>
<div class="toolbar">
<select id="countryFilter"><option value="">节点�?/option></select>
<input type="text" id="excludeFilter" placeholder="排除IP前缀, �?203.104" oninput="renderTable()" />
<input type="text" id="searchInput" placeholder="搜索节点名称、IP 或标�?.." oninput="renderTable()" />
...
<table>
<thead><tr><th>节点名称</th><th>国家</th><th>出口 IP</th><th>ASN</th><th>延迟</th><th>节点速度</th><th>状�?/th><th>操作</th></tr></thead>
...
<div class="form-group"><label class="form-label">用户�?/label><input class="input-field" id="credUsername" required /></div>
...
<div class="form-group"><label class="form-label">绑定域名（可选）</label><input class="input-field" id="credDomain" placeholder="�? admin.example.com" /></div>
<div class="form-group" style="display:flex;align-items:center;gap:8px"><label class="form-label" style="white-space:nowrap">启用 HTTPS</label><input type="checkbox" id="credHttps" style="width:18px;height:18px;accent-color:var(--accent)" /> <span style="font-size:11px;color:var(--text-tertiary)">需�?SSL 证书文件</span></div>
...
<div class="option-card active" data-value="auto" onclick="selectOptionCard('routingMode','auto')"><div class="option-card-title">智能路由</div><div class="option-card-desc">自动选择最�?IP</div></div>
<div class="option-card" data-value="fixed_region" onclick="selectOptionCard('routingMode','fixed_region')"><div class="option-card-title">区域锁定</div><div class="option-card-desc">锁定到指定国�?/div></div>
...
<div class="option-card active" data-value="all" onclick="selectOptionCard('routingIpType','all')"><div class="option-card-title">全部类型</div><div class="option-card-desc">IPv4 �?IPv6</div></div>
<div class="option-card" data-value="ipv4" onclick="selectOptionCard('routingIpType','ipv4')"><div class="option-card-title">�?IPv4</div><div class="option-card-desc">只使�?IPv4</div></div>
<div class="option-card" data-value="ipv6" onclick="selectOptionCard('routingIpType','ipv6')"><div class="option-card-title">�?IPv6</div><div class="option-card-desc">只使�?IPv6</div></div>
...
&#8226; <strong style="color:var(--text-primary)">Vultr</strong> �?全球 32 个数据中心，最�?$2.5/�?br/>
...
<div class="modal-title">网关服务状�?/div>
...
var COUNTRY_MAP = {
  'JP':'日本','US':'美国','SG':'新加�?,'KR':'韩国','GB':'英国','DE':'德国',
  'FR':'法国','CA':'加拿�?,'AU':'澳大利亚','NL':'荷兰','HK':'香港','TW':'台湾',
  'IN':'印度','BR':'巴西','RU':'俄罗�?,'CH':'瑞士','SE':'瑞典','NO':'挪威',
  'IT':'意大�?,'ES':'西班�?,'Unknown':'未知'
};
...
var COUNTRIES = [
  ...
  {name:'新加�?,short:'SG'},{name:'英国',short:'GB'},{name:'德国',short:'DE'},{name:'法国',short:'FR'},
  {name:'加拿�?,short:'CA'},{name:'澳大利亚',short:'AU'},{name:'荷兰',short:'NL'},{name:'香港',short:'HK'},
  {name:'台湾',short:'TW'},{name:'印度',short:'IN'},{name:'巴西',short:'BR'},{name:'俄罗�?,short:'RU'},
  {name:'瑞士',short:'CH'},{name:'瑞典',short:'SE'},{name:'挪威',short:'NO'},{name:'意大�?,short:'IT'},{name:'西班�?,short:'ES'}
];
...
for (var si = 0; si < 6; si++) {
  sampleChannels.push({
    ...
    online: false, connecting:false, country:['日本','美国','新加�?,'韩国','英国','德国'][si],
...
var sampleNodes = [
  ...
  {name:'新加�?· SG-1',country:'新加�?,ip:'103.25.59.10',asn:'AS45102',asn_org:'Tencent Cloud',latency:68,speed:32.1,status:'available'},
  {name:'新加�?· SG-2',country:'新加�?,ip:'103.25.59.18',asn:'AS45102',asn_org:'Tencent Cloud',latency:72,speed:29.8,status:'available'},
...
async function load() {
  try {
    ...
    $('systemStatus').innerHTML = '<span class=\"dot\"></span>系统运行�?;
    updateCountryFilter();
    renderChannels();
    renderTable();
  } catch(e) {
...
function updateCountryFilter() {
  ...
  var html = '<option value="">节点�?/option>';
  countries.forEach(function(c) {
    html += '<option value="'+esc(c)+'">'+esc(c)+' ('+countMap[c]+'个节�?</option>';
  });
  sel.innerHTML = html;
}
...
function buildAsnSelectForChannel(selected, countryFilter) {
  ...
  asns.forEach(function(a){ var cnt=asnMap[a]>1?"("+asnMap[a]+"�?":""; var s=(a===selected)?" selected":""; h+="<option value=\""+a+"\""+s+">"+a+cnt+"</option>"; });
...
function buildNodeSelectForChannel(countryFilter, asnFilter, selected) {
  ...
  if(!list.length) h += "<option value=\"\" disabled>无匹配节�?/option>";
  return h;
}
...
function renderChannels() {
  ...
  for (var i=0;i<Math.min(chList.length,6);i++) {
    ...
    html += '<div class="channel-card '+activeClass+'">'+
      ...
      '<div class="channel-card-footer"><div class="channel-conn-status"><span class="dot-sm '+(ch.online?'connected':'disconnected')+'"></span><span class="'+(ch.online?'text-connected':'text-disconnected')+'">'+(ch.online?'已连�?:'未连�?)+'</span></div>'+
...
function renderTable() {
  ...
  for (var i=0;i<list.length;i++) {
    ...
    else if (n.status==='unavailable'||n.status==='offline') { statusLabel='不可�?; statusBadgeClass='badge-unavailable'; }
    ...
    html += '<tr class="'+(isActive?'active-row':'')+'">'+
      ...
      '<button class="test-btn"'+(n.status==='unavailable'||n.status==='offline'||isTesting?' disabled':'')+' onclick="testNode(\''+esc(n.name)+'\')">'+(isTesting?'测速中...':'测�?)+'</button>'+
      '</div></td></tr>';
  }
...
async function connectChannel(idx) {
  ...
  if (!list.length) { alert('没有匹配的节点，请检查国�?ASN/节点选择'); return; }
  var target = list[0];
  ch.connecting = true;
  ch.online = false;
  renderChannels();
...
async function saveCredentials(e) {
  ...
  if (!username||(!password&&!(state&&state.password_set))) {
    errorEl.textContent='用户名不能为空；首次设置时密码不能为�?;
    errorEl.style.display='block'; return;
  }
  if (isNaN(port)||port<1||port>65535) {
    errorEl.textContent='端口范围必须�?1 �?65535 之间';
    errorEl.style.display='block'; return;
  }
  ...
  try {
    ...
    var d = await r.json();
    if (r.ok&&d.ok) {
      successEl.textContent=d.restart_needed?'保存成功！端口或路径已变更，页面将自动跳�?..':'保存成功，已即时生效�?;
      ...
      errorEl.textContent=d.error||'保存失败，请检查输�?;
      errorEl.style.display='block'; btn.disabled=false; btn.textContent='保存修改';
    }
  } catch(err) {
    errorEl.textContent='连接服务器失败，请稍后重�?;
    errorEl.style.display='block'; btn.disabled=false; btn.textContent='保存修改';
  }
}
...
async function saveNetwork(e) {
  ...
  if (isNaN(proxyPort)||proxyPort<1024||proxyPort>65535) {
    errorEl.textContent='代理出口端口范围必须�?1024 �?65535 之间'; errorEl.style.display='block'; return;
  }
  btn.disabled=true; btn.textContent='正在保存...';
  try {
    ...
    errorEl.textContent='连接服务器失�?; errorEl.style.display='block';
...
function renderGatewayServices(services) {
  ...
  services.forEach(function(s){
    var statusText = s.status==='running'?'正在运行':'已停�?;
...
def check_proxy_health() -> dict[str, Any]:
    ...
    try:
        ...
        diag_msg = diag[1] if diag else f"端口 {LOCAL_PROXY_PORT} 连接失败，原�? {e}"
        return {
            "ok": False,
            "error": f"代理服务未运�?({diag_msg})"
        }
    finally:
        if s is not None:
            try:
    ...
    # 3. 使用 curl 通过本地 SOCKS5 代理接口测试 IP 与实际延�?    def _curl_check_ip(url: str) -> dict[str, Any] | None:
        proxy_hosts = []
        if LOCAL_PROXY_HOST == "::":
    ...
    try:
        ...
        return {"ok": False, "error": "出口连接测试失败 (ip.sb �?api.ipify.org 均无法连通，可能是节点已失效�?VPS 防火墙限制了 UDP/TCP 出站端口)"}
    except Exception as e:
        ...
        try:
            ...
            if res["ok"]:
                ...
                if active_openvpn_node_id:
                    ...
                    log_to_json("WARNING", "Proxy", f"代理不可�? {error_msg}")
                set_state(
                    proxy_ok=False,
                    proxy_ip="-",
                    proxy_latency_ms=0,
                ...
                if active_openvpn_node_id:
                    ...
                    if routing_mode != "fixed_ip":
                        with lock:
                            ...
                            if active_node:
                                mark_blacklisted(active_node, f"代理连通性检测失�? {error_msg}")
            ...
            print(f"[错误] 代理后台检测发生异�? {e}", flush=True)
            log_to_json("ERROR", "Proxy", f"检测守护线程发生异�? {e}")
        time.sleep(30)
                ...
                if node:
                    ...
                    if ip:
                        ...
                        if latency > 0:
                            ...
                            set_state(active_node_latency="检测超�?)
                    else:
                        set_state(active_node_latency="检测超�?)
                else:
                    set_state(active_node_latency="检测超�?)
            elif is_connecting:
                set_state(active_node_latency="测试�?..")
            else:
                set_state(active_node_latency="无活动连�?)
        except Exception as e:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def is_authorized(self) -> bool:
        ...
        if not pwd:
            print("[Auth] 管理后台密码为空，已拒绝访问。请检�?ui_auth.json�?, flush=True)
            return False
    ...
    def read_request_body(self, max_bytes: int = 65536) -> bytes:
        ...
        if length > max_bytes:
            raise ValueError(f"请求体过大，最大允�?{max_bytes} 字节")
    ...
    def do_GET(self) -> None:
        ...
        if effective_path in ("/", "/index.html"):
            ...
            try:
                ...
                diag = vpn_utils.diagnose_local_obstructions(LOCAL_PROXY_PORT, host=LOCAL_PROXY_HOST)
                proxy_err = diag[1] if diag else f"本地代理网关无法连�? {e}"
            finally:
                if s is not None:
                    try:
                        s.close()
            ...
            ovpn_details = "未连�?
            if ovpn_ok:
                ovpn_details = f"已连接节�? {active_openvpn_node_id}"
            ...
            pinger_status = {
                ...
                "error": "" if pinger_ok else "线程可能已中止，无法实时刷新活动节点�?Ping 延迟�?
            }
            self.send_json({
                "ok": True,
                "services": [
    ...
    def do_POST(self) -> None:
        ...
        if effective_path == "/api/login":
            try:
                ...
                if expected_pwd and input_pwd == expected_pwd and input_uname == expected_uname:
                    ...
                    self.send_json({"ok": False, "error": "用户名或密码不正确，请重新输�?}, HTTPStatus.FORBIDDEN)
            except Exception as exc:
        ...
        if effective_path == "/api/update_credentials":
            try:
                ...
                if not new_username or (not new_password and not ui_cfg.get("password")):
                    self.send_json({"ok": False, "error": "用户名不能为空；首次设置时密码不能为�?}, HTTPStatus.BAD_REQUEST)
                    return
                ...
                try:
                    ...
                    self.send_json({"ok": False, "error": "网页管理端口范围必须�?1 �?65535"}, HTTPStatus.BAD_REQUEST)
                    return
            ...
            try:
                ...
                try:
                    ...
                    self.send_json({"ok": False, "error": "代理出站端口范围必须�?1024 �?65535"}, HTTPStatus.BAD_REQUEST)
                    return
                
                if routing_mode not in ("auto", "fixed_ip", "fixed_region", "favorites"):
                    self.send_json({"ok": False, "error": "无效的路由配置模�?}, HTTPStatus.BAD_REQUEST)
                    return
                ...
                if new_proxy_port_int == ui_cfg.get("port", 8787):
                    self.send_json({"ok": False, "error": "代理出站端口不能与网页管理端口相�?}, HTTPStatus.BAD_REQUEST)
                    return
        ...
        if effective_path == "/api/check":
            ...
            try:
                ...
                last_active_ping_time = 0.0
                last_active_latency = 0
                set_state(active_openvpn_node_id="", last_check_message="手动断开连接", active_node_latency="无活动连�?)
...
def main() -> None:
    ...
    write_json(
        STATE_FILE,
        {
            ...
            "last_check_message": "服务已启动，正在初始化网络并获取候�?VPN 节点...",
            "is_connecting": True,
    ...
    if gateway_ready:
        print("[网关] 代理网关已成功启动监听，启动同步与检测脚�?..", flush=True)
    else:
        print("[警告] 代理网关启动超时，继续执行脚�?..", flush=True)
